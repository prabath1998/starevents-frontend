export default function Button({ disabled, children, variant='primary', className='', ...props }) {
  ...
  return <button disabled={disabled} className={`${base} ${variants[variant]} ${className}`} {...props}>{children}</button>
}
