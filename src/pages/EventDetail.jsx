...
const DEFAULT_EVENT_IMAGE = 'https://images.unsplash.com/photo-1514525253161-7a46d19cd819?q=80&w=2940&auto=format&fit=crop';
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">         
          ...
          <div className="md:col-span-1">
            ...
            <div className="space-y-4">
              {types.length > 0 ? (
                types.map(t => (
                  <Card key={t.id} className="p-6 bg-gray-800/60 border border-gray-700 backdrop-blur-sm shadow-lg">
                    ...
                    <div className="mt-auto">
                      ...
                      <div className="mt-4 flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          <Button variant="secondary" onClick={() => add(t.id, -1)} disabled={(qty[t.id] || 0) === 0} className="rounded-full w-8 h-8 flex items-center justify-center">
                            <MinusIcon className="w-4 h-4" />-
                          </Button>
                          <span className="w-10 text-center text-lg text-white font-semibold">{qty[t.id] || 0}</span>
                          <Button onClick={() => add(t.id, 1)} className="rounded-full w-8 h-8 flex items-center justify-center">
                ...
                <div className="text-center text-gray-500 py-4">No tickets are currently available for this event.</div>
              )}
            </div>
          </div>
        </div>
